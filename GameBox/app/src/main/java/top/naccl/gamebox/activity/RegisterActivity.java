...
public class RegisterActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		register_btn.setOnClickListener(registerButtonListener);
	}
	...
	private boolean checkEditText(String username, String password) {
		if (username.length() < 3 || username.length() > 10) {
			ToastUtils.showToast(RegisterActivity.this, "用户名长度为3-10个字符！");
			return false;
		}
		if (password.length() < 6 || password.length() > 16) {
			ToastUtils.showToast(RegisterActivity.this, "密码长度为6-16个字符！");
			return false;
		}
		return true;
	}
	...
	private void postRegister(String username, String password) {
		RequestBody requestBody = new FormBody.Builder()
				...
				.build();
		OkHttpUtils.postRequest(ApiConfig.REGISTER_URL, requestBody, new Callback() {
			@Override
			...
			public void onFailure(@NotNull Call call, @NotNull IOException e) {
				ToastUtils.showToast(RegisterActivity.this, "请求失败");
			}
		});
	}
}
